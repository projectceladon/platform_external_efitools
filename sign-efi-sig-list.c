...
help(const char *progname)
{
	...
	printf("Produce an output file with an authentication header for direct\n"
	       ...
	       "\t                 <timestamp> must follow the format \"%s\"\n"
...
main(int argc, char *argv[])
{
	...
	if (timestampstr) {
		char *c;
		c = strptime(timestampstr, TIMEFMT, &tms);
		if (c == NULL || *c != '\0') {
			fprintf(stderr, "Failed to parse time '%s'. Required format is %s\n", timestampstr, TIMEFMT);
			exit(1);
		}
...
#endif
	int fdoutfile = open(outfile, flags, S_IWUSR|S_IRUSR);
	if (fdoutfile == -1) {
